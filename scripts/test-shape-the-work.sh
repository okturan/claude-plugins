...
FAKE_OPENSPEC_ROOT="$project_root/services" json_output="$(run_checker)"
assert_status "$json_output" "openspec-explore" "ready"

mkdir -p "$project_root/another-service"
FAKE_OPENSPEC_ROOT="$project_root/another-service" json_output="$(run_checker)"
assert_status "$json_output" "openspec-explore" "needs-context-check"
assert_contains "$json_output" "does not contain target directory $start_directory"
...
assert_status "$json_output" "openspec-explore" "needs-context-check"
assert_contains "$json_output" "is outside target Git root $project_root"
